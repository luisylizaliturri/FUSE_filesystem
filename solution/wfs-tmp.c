...
int allocate_inode() {
    for (size_t disk = 0; disk < num_disks; disk++) {
        char *inode_bitmap = (char *)disk_maps[disk] + superblock.i_bitmap_ptr;
        for (int i = 0; i < superblock.num_inodes; i++) {
            if (!(inode_bitmap[i / 8] & (1 << (i % 8)))) {
                // Mark as allocated on all disks
                for (size_t d = 0; d < num_disks; d++) {
                    char *disk_inode_bitmap = (char *)disk_maps[d] + superblock.i_bitmap_ptr;
                    disk_inode_bitmap[i / 8] |= (1 << (i % 8));
                }
                return i;
            }
        }
...
int wfs_getattr(const char *path, struct stat *stbuf) {
    ...
    //otherwise locate the inode for the given path
    int inode_num = find_inode_by_path(path);
    printf("wfs_getattr(): inode_num: %d\n", inode_num);
    ...
    printf("wfs_getattr(): st_uid: %d\n", stbuf->st_uid);
    printf("wfs_getattr(): st_gid: %d\n", stbuf->st_gid);
    printf("wfs_getattr(): st_atime: %ld\n", stbuf->st_atime);
    printf("wfs_getattr(): st_mtime: %ld\n", stbuf->st_mtime);
    printf("wfs_getattr(): st_mode: %d\n", stbuf->st_mode);
    printf("wfs_getattr(): st_size: %ld\n", stbuf->st_size);
...
int wfs_mkdir(const char *path, mode_t mode) {
    ...
    // Allocate data block for parent if needed
    if (parent_inode->blocks[0] == 0) {
        ...
        // Update parent's block allocation on all disks
        for (size_t i = 0; i < num_disks; i++) {
            char *block_bitmap = (char *)disk_maps[i] + superblock.d_bitmap_ptr;
            block_bitmap[parent_block_num / 8] |= (1 << (parent_block_num % 8));
        }
    }
    ...
    printf("Directory entry added to parent inode.\n");

    // Initialize the new inode
    struct wfs_inode new_inode = {0};
    new_inode.num = new_inode_num; // Set the correct inode number
    new_inode.mode = S_IFDIR | mode;
    new_inode.uid = getuid();
    new_inode.gid = getgid();
    new_inode.size = 0;
    new_inode.nlinks = 2; // Set to 2 for "." and ".."
...
void parse_args(int argc, char *argv[]) {
    if (argc < 3) { // ./wfs disk1 disk2 -s mount_point
...
int main(int argc, char *argv[]) {
    parse_args(argc, argv); //store disk files in disk_files and num_disks
